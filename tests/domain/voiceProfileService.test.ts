...
describe("VoiceProfileService", () => {
  it("refuses to analyze with fewer posts than the configured minimum", async () => {
    const { container } = createTestSetup(3);
    const user = container.users.getOrCreate("chat-1");
    container.posts.add(user.id, "just one post");
  ...
  it("builds and stores a voice profile once enough posts are present", async () => {
    const { container, ai } = createTestSetup(2);
    const user = container.users.getOrCreate("chat-1");
    container.posts.add(user.id, "First post about formulation.");
    container.posts.add(user.id, "Second post about clean beauty claims.");
    ...
    const active = container.voiceProfiles.getActive(user.id);
    expect(active?.id).toBe(record.id);
  });

  it("deactivates the previous profile when a new one is saved", async () => {
    const { container, ai } = createTestSetup(1);
    const user = container.users.getOrCreate("chat-1");
    container.posts.add(user.id, "A post.");
    ...
    const all = container.voiceProfiles.listByUser(user.id);
    expect(all).toHaveLength(2);
    expect(container.voiceProfiles.getActive(user.id)?.id).toBe(second.id);
    expect(first.id).not.toBe(second.id);
  });

  it("throws MissingVoiceProfileError when no active profile exists", () => {
    const { container } = createTestSetup(1);
    const user = container.users.getOrCreate("chat-1");
    expect(() => container.voiceProfileService.getActiveOrThrow(user.id)).toThrow(MissingVoiceProfileError);
  });
});
